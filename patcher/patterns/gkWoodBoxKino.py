...
string_section_start = PatchPattern(
    ...
    patternJP=[
        Instruction(
            identifier=1, offset=0x0, pattern=parse_pattern_bytes("47 6b 57 6f 6f 64 42 6f 78 4b 69 6e 6f 00"),
...
eventManager = PatchPattern(
    ...
    patternJP=[
        Instruction(
            identifier=1, offset=0x0, pattern=parse_pattern_bytes("45 76 65 6e 74 4d 61 6e 61 67 65 72 00"),
    ...
    patchMapJP=[
        Patch(
            ...
            patch_function=lambda offset, data, plando_dict, patch_patterns, pattern_name: (
                0x663031303154616c6b4d616e6b657900).to_bytes(16, 'big'),
...
shroomish_box_wait = PatchPattern(
    ...
    patternJP=[
        ...
        Instruction(
            identifier=2, offset=0x38, pattern=parse_pattern_bytes("ff fd 00 0c"),
            instruction_readable="store_arg -0x3"
        ),
        Instruction(
            identifier=3, offset=0x3c, pattern=parse_pattern_bytes("ff fe 00 0b"),
            instruction_readable="load_arg -0x2"
        ),
        Instruction(
            identifier=4, offset=0x40, pattern=parse_pattern_bytes("ff fd 00 0b"),
            instruction_readable="load_arg -0x3"
        ),
        Instruction(
            identifier=5, offset=0x44, pattern=parse_pattern_bytes("00 29 00 10"),
            instruction_readable="push 0x29"
        ),
        Instruction(
            identifier=6, offset=0x48, pattern=parse_pattern_bytes("00 15 03 01"),

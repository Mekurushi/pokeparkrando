...
with open(f"{REGION}/original_symbols.txt", "r") as f:
    original_symbols = yaml.safe_load(f)

with open(f"{REGION}/free_space_start_offsets.txt", "r") as f:
...
try:
    ...
    all_asm_file_paths = glob.glob(f"{REGION}/patches/*.asm")
    ...
    for patch_filename in all_asm_files:
        ...
        patch_path = os.path.join("", f"{REGION}/patches", patch_filename)
